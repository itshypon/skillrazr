...
const AddQuestionForm = ({
  ...
  const handleQuestionTitleChange = (event: any) => {
    ...
    checkFormErrors(
      qD,
      quizData.answers[
        editingQuestionId ? editingQuestionId : quizData.questions.length + 1
      ]
    );
  };
  ...
  const checkFormErrors = (_questionData: any, answer: string) => {
    const validAnswer = answer && _questionData.options.includes(answer);
    ...
    if (
      formFilled &&
      new Set(_questionData.options).size === 4 &&
      validAnswer
    ) {
  ...
  const handleAddQuestion = () => {
    if (
      ...
      setOneQuestionData(
        editingQuestionId
          ? { ...questionData }
          : { ...questionData, id: `${quizData.questions.length + 1}` },
        editingQuestionId
      );
  ...
  return (
    ...
    >
      <DialogTitle id="max-width-dialog-title">
        <div>
          {editingQuestionId
            ? `Edit details for Question ${editingQuestionId}`
            : `Add details for Question ${quizData.questions.length + 1}`}
        </div>
        {invalidAnswer && (
