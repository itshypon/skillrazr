...
const RenderTestCoverageContent = (props: any) => {
    ...
    const setUpTests2 = () => {
        ...
        describe('deliverableItems()', function () {
            let deliverableItems = (user: any, inventory: any, day: string) => {
                ...
                const isAvailable = itemsSubscribed.filter((subItem) => {
                    const itemInventory = inventory.find((invItem: any) => invItem.id === subItem.id);
    ...
    return (
        <div className='text-xl flex flex-col items-center w-full'>
            ...
            <img alt='unit testing' src="https://images.unsplash.com/photo-1576444356170-66073046b1bc?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1740&q=80" loading="lazy" role="presentation" width="700" height="467"></img>
            <div className="flex justify-center m-auto">
                <span className='mr-1 text-sm'>Photo by Ferenc Almasi </span> <a href="https://unsplash.com/?utm_source=medium&utm_medium=referral" className='text-sm'>Unsplash</a> </div>
            <br />
            ...
            <div className='mt-2'>
                Good working software often consists of statements, methods, classes and modules. Code is kept in units for better organisation, readability and maintenability and for testability where smaller units of code can be tested easily. Code is often subject to changes. Code is refactored to maintain quality and new changes introduced to release new feautures or fix existing bugs. Unit tests plays an important role to catch bugs introduced because of these. Test cases ensure the code is working as expected. Once unit tests are written, developers can introduce new changes to code confidently, as they can run the unit test scenarios to catch bugs if any.
                Mostly developers who code the program write unit tests.
            </div>

            <div className="my-4">
                In this article we'll write unit tests for a real world scenario. <br /> <br />
                We need a utility function that will be used in one App <span className='italic'>Grocery Daily </span>, to deliver items a customer has subscribed for a given day.
                This function receives 3 input parameters namely user (an object), inventory (an array of objects) and day a string
                and returns an object with a property named type whose values can be 'success' or 'error' and with additional properties, data an array of items in case of
                success or reason of an error in case of any error.
                <br /> <br />
                <span className='italic'>Criterias :- <br /></span>
                User object will have atleast below properties:- <br />
                1. Subscriptions with arrays of objects with itemId, itemQuantity and subscribed days array. <br />
                2. wallet balance (available amount) <br /> <br />

                When inventory is empty or partially available, you can return 'Sorry, items not available' and keep users wallet balance intact. <br />
                When no items subscribed for a particular day, return a message 'No items subscribed today!' <br />
                Finally return the subscribed items, assuming we've covered all the cases. <br />
                ...
                Let's write code to meet this above requirement.
                We'll write proper unit tests that ensures our code is working as expected.
                For this we'll use TDD(Test Driven Development). In TDD process, we write the test cases first before writing a single line of code.
                <div className='mt-2'>
                    Here we are using <a className="text-xl underline" href="https://mochajs.org/" target={"_blank"} rel="noreferrer">Mocha</a> A test framework that helps to run our test cases in the browser. Later we' ll run our test cases right here in this page and see the results of the test run. We'll also need an assertion library, that'll help us write better assertion statements. e.g expect(1+1).to.equal(2). We'll be using <a href="https://www.chaijs.com/" className="text-xl underline" rel="noreferrer" target={"_blank"}>Chai.</a>
                </div>
                <div className='my-2'>
                    There are many frameworks and assertion libraries but we've picked Mocha and Chai to run the test cases on the browser.</div>
            </div>

            <div className='w-full mt-2'>
                {/* style="white-space: pre-wrap; background: lightgray;" */}
                <div className='my-4'>Here goes our first unit test, with following test case in mind.</div>
                User has wallet balance of 10 coins. User has subscribed Apples of quantity 2 at price 40 coins each for Sun and Tue and Mangoes of quantity 1 at price 20 coins for Tue, Wed and Thu. Inventry has enough items. For Sun (Sunday) the method should return error message 'Insufficient Balance!'. We're adding a top level 'describe' block for the function and write each test scenerio in an 'it' block.
...
`}
                ...
                <div>Notice how we've kept the test data specific to match the test case inside the "it" block. We isolate test data that is specific to a test inside it's "it" block. Any common data that is used in all "it" blocks is kept at the top "describe" block. </div>
                <div className='my-2'>   Let's code the function so that it can be invoked in the tests, for now just a function that returns an empty object and no logic.</div>

                <pre className='p-4 my-8 rounded bg-gray-200 whitespace-pre-wrap overflow-auto'>
                    <code>
                        {`let deliverableItems = (user, inventory, day) => ({});`}</code></pre>
...
it('should return success with subscribed items for the given day', () => {
    ...
    expect(result.items[1].quantity).to.equal(1);
                ...
                <div className='my-4'>With no logic in the function's body, all test cases inside the test suite will fail.  </div>
                <div className='my-4'>Time to run our unit tests! Click Run Tests, behind the scene it registers the test cases in the test suite and instructs Mocha to run the suite and
                    we get a nice report.
                    <span className='italic'> Let's Run!</span></div>
                <Button variant='contained' onClick={runTests1} className='text-xl !my-8 !block'>Run Tests</Button>
                <div className='my-4'> All failed! It was supposed to fail. Let's fix all the test cases by writing the logic for the function. </div>
                <div className='my-2'> While writing the code for the function we try to fix all the test cases. When the test cases are valid and completely cover all the scenarios, writing code to pass all the test cases completes our initial coding task.</div>
    ...
    return {type: 'success', items: itemsSubscribed };
                ...
                <div >Now run tests to see test results again. This time all should pass. We should see the existing failures in <span className='text-red-500'>Red</span> becoming <span className='text-green-500'>Green</span>.</div>
                <Button variant='contained' onClick={runTests2} className='text-xl !my-8'>Run Tests</Button>
                <div className='my-4'>Once all tests pass, it's time to refactor the code to improve readability by following standard coding practices. As we keep changing the code, we run the test cases to detect potenital bugs.   </div>
                <div ><span className='font-extrabold'>We've missed one test case</span>, when user has no subscribed items for a given day, it's returning the desired message, but there's no test case to validate that. As a program has many statements, functions, condtional statements resulting in multiple branches and control flows without a code coverage report chances of missing such test cases are more.</div>
                <div className='text-2xl  my-8'>Code Coverage</div>
                <div>It's a metric to measure how much code is covered while tests are run against the code. A code coverage report highlights number of statements, functions, branches the test covered and helps in detecting missing tests that should be added. In this article we're not using any library to measure the code coverage. There are many popular libraries out there and it's fairly simple to add it to a testing framework.</div>

                <div className='text-2xl  my-8'>Industry Insights </div>
                <div>To demonstrate unit tests we've used Mocha on a Browser. But it's convenient to use a Node process to run the unit tests in case of JavaScript based environments. Most frameworks run it on watch mode, whenever there is a change in code, unit tests are run automatically. Also it's integrated with the code commits and CICD work flows where tests are triggered before code is committed or deployed to production.</div>
                <div className='my-4'>That's all about Unit Tests and Code Coverage. In our next article we'll discuss about how Unit Testing is not enough and how Integration Tests help.</div>
            </div>
            <ModalDialog
    ...
    const renderTechBlogContent = () => {
        return (
            <div className='text-xl flex flex-col items-center w-full'>
                ...
                <div className="flex justify-center m-auto">
                    <span className='mr-1 text-sm'>Photo by Razvan Chisu on </span> <a href="https://unsplash.com/?utm_source=medium&utm_medium=referral" className='text-sm'>Unsplash</a> </div>
                <br />
                ...
                <div className="p-4">
                    ...
                    <div className='text-xl my-4'> Who are they?</div>
                    ...
                    <div className='text-2xl my-8'>25. Administration & Support staff :-</div>
                    ...
                    To write this article information from various websites gathered, analysed, modified and presented in a format suitable for this article. It's not possible to credit all 100's of such sources. A big thanks to the Internet, abundance of information.
                    Now it's time to take a break! We'll revisit this article periodically and update any missing info that'll add value.
        ...
        <>
            ...
            <div className={`flex flex-col w-full mt-10 py-20 px-10 sm:px-20`}>
                <div className="flex items-center flex-col">
                    ...
                    {!blog ? <div className='flex flex-col items-center p-20'>
                        <span className='text-red-500 text-6xl'>
                            <span>4</span><span>0</span><span>4</span></span>
                        <span className='flex items-center text-2xl text-red-500'> <ErrorOutlineIcon htmlColor='red' /> <span className='ml-1'>Blog not found!</span></span>
                    </div> : renderBlogContent()
                    }
                    <div className='w-full my-8 text-center pt-6'><NavLink to="/blogs"><Button variant='outlined'><ArrowBackIcon /> Back to Blogs List </Button> </NavLink></div>
                </div>
            </div >
        </>
    );
